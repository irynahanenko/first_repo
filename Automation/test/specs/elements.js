...
describe('Validate Elements section', () => {
    ...
    it('Validate placeholder texts of inputs', () => {
        expect(TextBox.inputFullNamePlaceholder).to.equal(labels.textBoxPlaceholders.fullName);
        expect(TextBox.inputEmailPlaceholder).to.equal(labels.textBoxPlaceholders.email);
    ...
    it('Fill the form, click Submit btn and validate the result', () => {
        ...
        expect(TextBox.resultEmail.getText().split(':')[1]).to.equal(inputTextBox.email);
        expect(TextBox.resultCurrentAddress.getText().split(':')[1]).to.equal(inputTextBox.currentAddress);
        expect(TextBox.resultPermanentAddress.getText().split(':')[1]).to.equal(inputTextBox.permanentAddress);
    });
